  ...
  <br><br>
   ...
   <nav class="navbar navbar-light bg-light d-flex justify-content-center" >
		  <div class="container-center" >
						<form  class="d-flex justify-content-center" id='searchForm' action="/admin/clientListYPaging" method='get'>
        ...
        <form id= "clientList" method="get">
      	...
      	<div class="row">
			<table class="table table-striped table-bordered table-hover">
			    ...
			    <tbody>
			    	 <c:forEach items="${getClientListY }" var="clientY">
                                  <tr>
	                                <td><c:out value="${clientY.cid }"  /></td>
	                                <td><c:out value="${clientY.cname }"/></td>
	                                <td><c:out value="${clientY.cnick }"/></td>
	                                <td><c:out value="${clientY.cphone }"/></td>
	                                <td><c:out value="${clientY.caddress }"/></td>
	                                <td><c:out value="${clientY.creport }"/></td>
	                                <td><c:out value="${clientY.cdelete }"/></td>
			                        <td><c:out value="${clientY.cgrade}"/></td>
		                            <td><fmt:formatDate pattern="yy/MM/dd" value="${clientY.cjoinDate }"/></td>
		                            <td>  
		                            	<button type="button" class='form-control btn btn-danger me-2'onclick="location.href='/admin/clientDeleteY?cid=${clientY.cid}'">삭제</button>   </td>
		...
		<nav aria-label="pagination">
      	...
      	<!-- 페이지번호표시 -->
				 ...
				 <form id="actionForm" action="/admin/clientListYPaging" method="get">
					 <input type="hidden" name="pageNum" value="${pageMaker.cri.pageNum}">
					 <input type="hidden" name="amount" value="${pageMaker.cri.amount}">
