  ...
  <br><br>
         <nav class="navbar navbar-light bg-light d-flex justify-content-center" >
		  <div class="container-center" >
						<form  class="d-flex justify-content-center" id='searchForm' action="/admin/saleList" method='get'>
						<select class="form-control me-2" name="type">
								...
								<option value="A"
									<c:out value="${pageMaker.cri.type eq 'A'?'selected':''}"/>>제목</option>
								<option value="IT"
									<c:out value="${pageMaker.cri.type eq 'IT'?'selected':''}"/>>아이디   or 기간</option>
								<option value="IA"
									<c:out value="${pageMaker.cri.type eq 'IA'?'selected':''}"/>>아이디   or 제목</option>
								<option value="ITA"
									<c:out value="${pageMaker.cri.type eq 'ITA'?'selected':''}"/>>아이디 or 기간 or 제목</option>
							</select> 
			...
			<table class="table table-striped table-bordered table-hover">
			    ...
			    <tbody>
			    	 <c:forEach items="${saleList }" var="saleList">
                                  <tr>
	                                <td><c:out value="${saleList.sno }"  /></td>
	                                <td><c:out value="${saleList.vid }"/></td>
	                                <td><c:out value="${saleList.stitle }"/></td>
	                                <td><c:out value="${saleList.sinfo }"/></td>
	                                <td><c:out value="${saleList.sprice }"/></td>
	                                <td><c:out value="${saleList.samount }"/></td>
		                            <td><fmt:formatDate pattern="yy/MM/dd" value="${saleList.swriteDate }"/></td>
		                            <td> 
		                            	<button type="button" class='form-control btn btn-danger me-2'onclick="location.href='/admin/saleListDelete?sno=${saleList.sno}'">삭제</button>  	
		                             </td>
		...
		<nav aria-label="pagination">
		  	<ul class="pagination justify-content-center">
				...
				<!-- 페이지번호표시 -->
				 ...
				 <form id="actionForm" action="/admin/saleList" method="get">
					 <input type="hidden" name="pageNum" value="${pageMaker.cri.pageNum}">
					 <input type="hidden" name="amount" value="${pageMaker.cri.amount}">
