	...
	<br><br> 
	  <nav class="navbar navbar-light bg-light d-flex justify-content-center" >
		  <div class="container-center" >
						<form  class="d-flex justify-content-center" id='searchForm' action="/admin/vendorListRPaging" method='get'>
        ...
        <form id= "vendorList" method="get">
      	...
      	<div class="row">
			<table class="table table-striped table-bordered table-hover">
			    ...
			    <tbody>
			    	 <c:forEach items="${vendorListRPaging }" var="vendorR">
                                  <tr>
	                                <td><c:out value="${vendorR.vid }"/></td>
	                                <td><c:out value="${vendorR.vname }"/></td>
	                                <td><c:out value="${vendorR.vregisterNo }"/></td>
	                                <td><c:out value="${vendorR.vphone }"/></td>
	                                <td><c:out value="${vendorR.vaddress }"/></td>
	                                <td><fmt:formatDate pattern="yyyy-MM-dd" value="${vendorR.vjoinDate }"/></td>
	                                <td><c:out value="${vendorR.vinfo }"/></td>
	                                <td><c:out value="${vendorR.vreport }"/></td>
	                                <td><c:out value="${vendorR.vdelete }"/></td>
			                        <td><c:out value="${vendorR.vgrade}"/></td>
		                             <td> 
		                             <button type="button" class='form-control btn btn-danger me-2'onclick="location.href='/admin/vendortDeleteR?vid=${vendorR.vid}'">삭제</button> 
		                             </td>
