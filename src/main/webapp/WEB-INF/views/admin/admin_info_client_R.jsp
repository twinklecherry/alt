...
<br><br>
 ...
 <nav class="navbar navbar-light bg-light d-flex justify-content-center" >
		  <div class="container-center" >
						<form  class="d-flex justify-content-center" id='searchForm' action="/admin/clientListRPaging" method='get'>
 ...
 <%-- <td><a class='toGet' href='<c:out value="${notice.nno }"/>'><c:out value="${notice.ntitle }"/></a></td> 
 <td><a class='toGet' href='<c:out value="/admin/clientDeleteR?cid=${clientR.cid}"/>'><c:out value="${notice.ntitle }"/></a></td>    --%>
					</tr>
			    </thead>
			    <tbody>
			    	 <c:forEach items="${getClientListR }" var="clientR">
                                  <tr>
	                                <td><c:out value="${clientR.cid }"  /></td>
	                                <td><c:out value="${clientR.cname }"/></td>
	                                <td><c:out value="${clientR.cnick }"/></td>
	                                <td><c:out value="${clientR.cphone }"/></td>
	                                <td><c:out value="${clientR.caddress }"/></td>
	                                <td><c:out value="${clientR.creport }"/></td>
	                                <td><c:out value="${clientR.cdelete }"/></td>
			                        <td><c:out value="${clientR.cgrade}"/></td>
		                            <td><fmt:formatDate pattern="yy/MM/dd" value="${clientR.cjoinDate }"/></td>
		                            
		                       <td> <button type="button" class='form-control btn btn-danger me-2'onclick="location.href='/admin/clientDeleteR?cid=${clientR.cid}'">삭제</button> </td>
		...
		<nav aria-label="pagination">
      	...
      	<!-- 페이지번호표시 -->
				 ...
				 <form id="actionForm" action="/admin/clientListRPaging" method="get">
					 <input type="hidden" name="pageNum" value="${pageMaker.cri.pageNum}">
					 <input type="hidden" name="amount" value="${pageMaker.cri.amount}">
